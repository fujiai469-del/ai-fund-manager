...
✅ 各銘柄に具体的な目標株価を設定（例：トヨタ ¥3,200 → 上昇余地+12%）
✅ 投資判断は必ず5段階で明示（Strong Buy/Buy/Hold/Sell/Strong Sell）
✅ リバランス指示は具体的な金額と株数で（例：ソニー ¥50,000分（約5株）売却 → 三菱UFJに振替）
✅ シナリオ分析は確率と金額を必ず記載

【数値表示ルール - 厳守】
⚠️ 金額は必ず整数で表示（端数切り捨て）：¥10,875 ではなく ¥10,000 や ¥11,000
⚠️ パーセンテージは小数点1桁まで：+12.345% ではなく +12.3%
⚠️ 売買推奨には必ず概算株数を併記：¥50,000分売却 ではなく ¥50,000分（約5株）売却
